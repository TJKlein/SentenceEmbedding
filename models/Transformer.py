...
class Transformer(nn.Module):
    ...
    def forward(self, features):
        ...
        output_states = self.auto_model(
            **trans_features, output_attentions=True,  return_dict=False)
        output_tokens = output_states[0]

        attn = format_attention(output_states[2])

        features.update({'token_embeddings': output_tokens, 'attention_mask': features['attention_mask'], "attention_tensor": attn})
